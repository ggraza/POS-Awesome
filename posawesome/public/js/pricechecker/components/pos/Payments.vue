        ...
        <v-row v-if="invoice_doc" class="px-1 py-0">
          ...
          <v-col cols="5">
            <v-text-field
              ...
              :background-color="frappe._(diff_color)"
              hide-details
...
export default {
  ...
  methods: {
  generateFBR(invoice_doc) {
    console.log("Generating FBR for invoice:", this.invoice_doc);
    var item_list = [];
    var total_qty = 0;
    var tax_rate = (this.invoice_doc.taxes.length > 0 ? this.invoice_doc.taxes[0].rate : 0);
    ...
    var fbrdata = {
      "InvoiceNumber": "",
      "POSID": this.invoice_doc.pos_id,
      "USIN": this.invoice_doc.name,
      "DateTime": this.invoice_doc.posting_date,
      "BuyerNTN": this.invoice_doc.ntn_no,
      "BuyerCNIC": null,
      "BuyerName": this.invoice_doc.customer,
      "BuyerPhoneNumber": null,
      "TotalBillAmount": this.invoice_doc.grand_total,
      "TotalQuantity": total_qty,
      "TotalSaleValue": this.invoice_doc.net_total,
      "TotalTaxCharged": this.invoice_doc.total_taxes_and_charges,
      "Discount": "0",
      "FurtherTax": "0",
      "PaymentMode": 1,
      "RefUSIN": null,
    ...
    $.ajax('https://gw.fbr.gov.pk/imsp/v1/api/Live/PostData', {
      type: 'POST',
      data: JSON.stringify(fbrdata),
      contentType: 'application/json',
      async: false,
      success: function (data, status, xhr) {
        console.log('FBR API response:', data);
        var invoiceNumber = data.InvoiceNumber;

        frappe.call({
          method: "posawesome.events.sales_invoice.set_invoice_number",
          args: {
          ...
          async: false,
          callback: function (e) {
            console.log('Invoice number set successfully');
          }
        });

      },
      beforeSend: function (xhr) {
        xhr.setRequestHeader("Authorization", "Bearer " + vm.settings.fbr_pos_token);
      },
    ...
    updateCreditSaleStatus() {
      if (this.pos_profile.posa_allow_credit_sale && !this.invoice_doc.is_return) {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            ...
            this.addresses = [];
            this.generateFBR(invoice_doc);
              //if (this.$root) {
              //console.log("Emitting generateFBR event with invoice_doc:", this.invoice_doc);
                //this.$root.$emit('generateFBR', this.invoice_doc);
              //evntBus.$emit("generateFBR", this.invoice_doc);
              //EventBus.$emit('generateFBR', this.invoice_doc);
            //}
          }
        },
      });
    ...
    shortSubmit(e) {
      if (event.key === 'F8' || e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
        e.preventDefault();
  ...
  computed: {
    ...
    diff_color() {
      return this.diff_payment > 0 ? "warning" : "white";
    },
  ...
  created() {
    ...
    document.addEventListener("keydown", this.shortCancel);
    document.addEventListener("keydown", this.shortSubmit);
  },
  beforeDestroy() {
  ...
  destroyed() {
    ...
    document.removeEventListener("keydown", this.shortCancel);
    document.removeEventListener("keydown", this.shortSubmit);
  },
  ...
  watch: {
    ...
    invoice_doc: {
      handler() {
        console.log("invoice_doc changed:", this.invoice_doc);
        this.updateCreditSaleStatus();
      },
      deep: true
    },
    ...
    is_credit_sale(value) {
      console.log("is_credit_sale changed:", value);
      if (value == true) {
        console.log("Credit sale enabled, setting payments to zero");
        this.invoice_doc.payments.forEach((payment) => {
          payment.amount = 0;
          payment.base_amount = 0;
        });
