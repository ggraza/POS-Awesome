<template>
  <div>
      ...
      <v-row class="items px-2 py-0">
        ...
        >
          <v-btn-toggle
            ...
            rounded
          >
            <v-btn small @click="show_coupons">{{ couponsCount }} {{ __('Coupons') }}</v-btn>
            <v-btn small @click="show_offers">{{ offersCount }} {{ __('Offers') }} : {{ appliedOffersCount }} {{ __('Applied') }}</v-btn>
          </v-btn-toggle>
        </v-col>
        <v-col
          cols="12"
        ...
        >
          <div class="d-flex justify-center align-center">
            {{ todayCustomerCount }} {{ __('Visit') }} of {{ customerCount }} {{ __('Outlets') }}
          </div>
        </v-col>
      </v-row>
      <v-row class="items px-2 py-0">
    ...
    <v-row align="center" class="items px-0 py-1 mt-0 pt-0" v-show="!dialog">
      <v-col
      ...
      <div class="my-0 py-0 overflow-y-auto" style="min-height: 30vh">
    ...
    <v-card class="cards mb-0 mt-3 py-0 grey lighten-5">
      <v-row no-gutters>
        <v-col class="items px-2 py-1 mt-0 pt-0" align="center" justify="center">
...
export default {
  ...
  methods: {
    ...
    getTodayCustomerCount() {
      frappe.call({
        method: 'posawesome.posawesome.api.customer.get_today_customer_count',
        callback: (r) => {
          if (r.message) {
            this.todayCustomerCount = r.message;
          }
        },
      });
    },
  ...
  created() {
    evntBus.$on('update_customer_count', (count) => {
  ...
  destroyed() {
    evntBus.$off('update_customer_count');
...
/deep/ .v-data-table__wrapper .v-data-table__mobile-row {
  height: auto;
...
.custom-data-table {
  ...
  /* Default styles for the data table */
}
