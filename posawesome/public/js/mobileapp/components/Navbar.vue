<template>
  <nav>
    <v-app-bar app height="40" class="elevation-2">
      ...
      ></v-app-bar-nav-icon>
      <v-img
        src="/assets/posawesome/js/mobileapp/components/pos/pos.png"
        alt="G2Virtu"
        @click="go_desk"
      ...
      >
        <span class="font-weight-light">G2</span>
        <span>Virtu</span>
      </v-toolbar-title>
...
<v-list-item @click="clearCache">
  ...
  <v-list-item-content>
    <v-list-item-title>{{ __('Clear Cache') }}</v-list-item-title>
  </v-list-item-content>
</v-list-item>
...
export default {
  ...
  clearCache() {
    ...
    caches.keys().then(function (names) {
      for (let name of names) caches.delete(name); // Clear service worker caches
    });
    this.show_mesage({ color: 'green', text: 'Cache Cleared' });
  setTimeout(() => {
    location.reload(); // Reload the page
  }, 500); // 0.5-second delay to ensure the message is shown
  },
    changePage(key) {
