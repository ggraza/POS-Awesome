...
export default {
  ...
  methods: {
    async updateFBRInvoice() {
      const item_list = [];
      let total_qty = 0;
      const tax_rate = this.invoice_doc.taxes.length > 0 ? this.invoice_doc.taxes[0].rate : 0;
      this.invoice_doc.items.forEach((item) => {
        const salevalue = item.rate * item.qty;
        item_list.push({
      ...
      try {
        console.log('Sending FBR invoice update request with data:', data);
        const response = await axios.post('https://gw.fbr.gov.pk/imsp/v1/api/Live/PostData', data, {
          headers: {
            'Authorization': `Bearer ${this.pos_profile.fbr_pos_token}`,
          },
        });
        console.log('FBR invoice updated successfully:', response.data);
        const invoiceNumber = response.data.InvoiceNumber;
        await this.$axios.post('/api/method/posawesome.events.sales_invoice.set_invoice_number', {
          doctype: this.invoice_doc.doctype,
          name: this.invoice_doc.name,
          inv: invoiceNumber,
        });
        console.log('Invoice number set successfully');
        this.$emit('refresh-doc');
      } catch (error) {
    ...
    async load_print_pages() {
      await this.updateFBRInvoice(); // Update FBR invoice before loading the print page
      const print_format =
        this.pos_profile.print_format_for_online ||
        this.pos_profile.print_format;
      const letter_head = this.pos_profile.letter_head || 0;
